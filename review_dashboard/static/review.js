...
// Add parameter isRoot to detect top-level objects for truncation
function renderHumanView(data, isRoot = false) {
  ...
  if (Array.isArray(data)) {
    ...
    if (isPrimitiveArray) {
      ...
      return '<div>' +
        data.map((item, idx) => `<div class="nested-card"><span class="human-label">Item #${idx + 1}</span>${renderHumanView(item, false)}</div>`).join('') +
        '</div>';
    }
  }
  if (typeof data === 'object') {
    ...
    for (const key of keys) {
      ...
      // If it's the top level object and we've hit 3 keys, split the grid
      if (isRoot && i === 3) {
        html += '</div><div class="human-grid more-fields" style="display: none; margin-top: 1.25rem;">';
        hasHidden = true;
      }

      html += `<div class="human-field">
                <span class="human-label">${escapeHtml(label)}</span>
                <div class="human-value">${renderHumanView(data[key], false)}</div>
            </div>`;
      i++;
    }
    html += '</div>';

    // Add "Show More" button if fields were hidden
    if (hasHidden) {
      html += `<button type="button" class="btn btn-edit" style="margin-top: 1rem; font-size: 0.75rem;" onclick="toggleMoreFields(this)">Show More</button>`;
    }
...
function toggleTableContent(table) {
  const content = document.getElementById(`content-${table}`);
  const indicator = document.getElementById(`indicator-${table}`);
...
function toggleApprovedRecords() {
  const isHidden = document.getElementById('toggle-approved').checked;
  const approvedCards = document.querySelectorAll('.record-status-approved');
...
function toggleMoreFields(btn) {
  const moreFieldsDiv = btn.previousElementSibling;
  if (moreFieldsDiv.style.display === 'none') {
    moreFieldsDiv.style.display = 'grid';
...
// ... [Keep your existing formatKeyLabel, escapeHtml, renderEditFields, toggleEdit, toggleRawView, prepareEditSubmission, setNestedValue unchanged] ...

// Render all records on page load
document.addEventListener("DOMContentLoaded", function () {
  // Apply initial hidden state for approved records
  toggleApprovedRecords();
  ...
  rawScripts.forEach(script => {
    ...
    try {
      ...
      if (viewContainer) {
        // Pass true here so it acts as the root object and cuts off after 3 elements
        viewContainer.innerHTML = renderHumanView(data, true);
      }
...
// Renders data into human-friendly cards limiting root objects to 3 key-value pairs initially
function renderHumanView(data, isRoot = false) {
  if (data === null || data === undefined) {
    return '<span class="empty-table-msg">N/A</span>';
  }
  ...
  if (typeof data === 'object') {
    ...
    // If rendering top-level item and keys exceed 3, split into visible and hidden fields
    if (isRoot && keys.length > 2) {
      const visibleKeys = keys.slice(0, 2);
      const hiddenKeys = keys.slice(2);
      ...
      html += '<div class="human-grid more-fields" style="display: none; margin-top: 1.25rem;">';
      ...
      html += `<button type="button" class="btn btn-edit" style="margin-top: 1rem; font-size: 0.75rem;" onclick="toggleMoreFields(this)">Show More (${hiddenKeys.length} more fields)</button>`;
...
// Toggles display of fields past the first 3
function toggleMoreFields(btn) {
  const moreFieldsDiv = btn.previousElementSibling;
  if (!moreFieldsDiv) return;

  const isHidden = moreFieldsDiv.style.display === 'none';
  moreFieldsDiv.style.display = isHidden ? 'grid' : 'none';

  const hiddenCount = moreFieldsDiv.children.length;
  btn.textContent = isHidden ? 'Show Less' : `Show More (${hiddenCount} more fields)`;
}

// Collapses/expands an entire table category
function toggleTableContent(table) {
  const content = document.getElementById(`content-${table}`);
  const indicator = document.getElementById(`indicator-${table}`);
  if (content.style.display === 'none') {
...
// Shows or hides all approved record cards on the page
function toggleApprovedRecords() {
  const isHidden = document.getElementById('toggle-approved').checked;
  const approvedCards = document.querySelectorAll('.record-status-approved');
  approvedCards.forEach(card => {
